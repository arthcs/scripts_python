...
#co2_average <- aggregate(CO2eq ~ Execution.Time ~ Treads, data = df_combined, FUN = calcular_media)
result <- aggregate(cbind(Execution.Time, CO2eq) ~ Treads, data = df_combined, FUN = calcular_media_sd)
