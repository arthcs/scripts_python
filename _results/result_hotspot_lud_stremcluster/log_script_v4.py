...
def process_data(file_path):
    ...
    for line in lines:
        ...
        # Tempo adicionado pelo python
        elif line.startswith("Tempo total:"):  # Parboil milisegundos
            #print(line)
        ...
        #EDITAR: encorar o tempo retornado pelo app
            ...
            entry['Time'] = float(line.split(':')[1].strip())
             ...
             seg = float(line.split(':')[1].replace('seconds', '').strip())
             entry['Time'] = seg #* 1000 # multiplicar por 1000 para converter para milissegundos
        elif line.startswith("Time for process: "): # kmeans
            entry['Time'] = float(line.split(':')[1].strip())
        elif line.startswith("Compute time: "): # cfd
            entry['Time'] = float(line.split(':')[1].strip())
        elif line.startswith("time ="): # streamcluster
            entry['Time'] = float(line.split('=')[1].strip())
            # backprop sem saida
            # rad Sem saida
